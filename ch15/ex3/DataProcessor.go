...
func parser(data []byte) (Input, error) {
	...
	if len(lines) < 4 {
		return Input{}, errors.New("wrong input: expected 4 lines")
	}
...
func NewController(out chan []byte) http.Handler {
	...
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		default:
			...
			w.Write([]byte("Too Busy: " + strconv.Itoa(rejected)))
			return
		}

		w.WriteHeader(http.StatusAccepted)
		w.Write([]byte("OK: " + strconv.Itoa(sent)))
	})
}
