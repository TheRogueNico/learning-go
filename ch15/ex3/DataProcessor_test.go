...
func FuzzParser(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, data []byte) {
		...
		if len(lines) < 4 {
			t.Fatalf("%q resulted in less than 4 lines", data)
		}
		if got.Id != string(lines[0]) {
			t.Errorf("Id = %q, want %q", got.Id, lines[0])
		}
		if got.Op != string(lines[1]) {
			t.Errorf("Op = %q, want %q", got.Op, lines[1])
		}
	})
}
